...
export function getMovieData(movieId) {
  return async dispatch => {
    ...
    const response = await axios.get(
      `https://api.themoviedb.org/3/movie/${movieId}?api_key=e0c15850977d1058ff053d4726ac46f1&language=en-US&append_to_response=credits,videos`
    );

    if (response.status >= 400) {
      dispatch(getMovieDataFailure(response.status_message));
      return;
    }
